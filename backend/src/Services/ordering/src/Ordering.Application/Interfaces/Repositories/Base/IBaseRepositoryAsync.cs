namespace Ordering.Application.Interfaces.Repositories.Base
{
    public interface IBaseRepositoryAsync<T, in TId> where T : class
    {
        ...
        IQueryable<T> GetPagedResponse(int pageNumber, int pageSize, CancellationToken cancellationToken = default);
