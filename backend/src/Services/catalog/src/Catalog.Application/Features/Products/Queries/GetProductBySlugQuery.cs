...
{
    public class GetProductBySlugQuery : IQuery<ProductItemResponse?>
    {
        public required string Slug { get; set; }
    }
    public sealed class GetProductBySlugQueryHandler : IQueryHandler<GetProductBySlugQuery, ProductItemResponse?>
    {
        private readonly IBaseRepositoryAsync<Product, string> _repo;

        public GetProductBySlugQueryHandler(IBaseRepositoryAsync<Product, string> repo)
        {
            _repo = repo;
        }

        public async ValueTask<ProductItemResponse?> Handle(GetProductBySlugQuery query, CancellationToken cancellationToken)
        {
            var product = await _repo.Entities
                ...
                .Include(c => c.ProductNears!)
                .ThenInclude(c => c.ProductNearItems)
